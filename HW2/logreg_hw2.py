...
def main():
    ...
    w, losses = trainLogistic(X_train_bias,y_train, 10000, 0.1)
...
def calculateNegativeLogLikelihood(X,y,w):
    nll = 0
    logit_z = logistic(X @ w)        
    result = y*np.log(logit_z+0.0000001) + (1-y)*np.log(1-logit_z+0.0000001)
...
def dummyAugment(X):
    l = len(X)
    column_v = np.ones((l, 1))
    aug_X = np.hstack((column_v, X))
