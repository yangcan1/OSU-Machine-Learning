...
number_of_layers = 4
width_of_layers = 17  # only matters if number of layers > 1
...
#   print("-------")
#   print("acc", val_accs[len(val_accs)-1])
  ...
  labels = np.array([np.argmax(softmax(logits), axis=1)])
#   print(labels)
  test_out = np.concatenate((np.expand_dims(np.array(range(4220),dtype=int), axis=1), labels.T), axis=1)
...
#   first_col = np.arange(len(y_pred_test)).reshape(-1, 1)
#   print(first_col.shape)
#   test_out = np.column_stack((first_col, y_pred_test))
#   test_out = np.concatenate((header, test_out))
#   np.savetxt('test_predicted.csv', test_out, fmt='%s', delimiter=',')
